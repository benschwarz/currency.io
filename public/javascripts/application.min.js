...
(a*this.rate).toFixed(2);a||(a=0);if(!/\./.test(a)&&(a.length>5||b.length>6))b=b.slice(0,-3);if(!(a.length>10||b.length>10)){if(/^\.$/.test(a))b="0.00";this.input.innerHTML=this.add_commas(a);this.output.innerHTML=this.add_commas(b)}},clear:function(){this.update_values("0")},add_commas:function(a){var b=/(\d+)(\d{3,3})/,c=(""+a).split(".");a=c[0];for(c=c[1]!==undefined?"."+c[1]:"";b.test(a);)a=a.replace(b,"$1,$2");return a+c},strip_commas:function(a){return a.replace(/,/g,"")}},buttons=$("#input-pad p"),
