...
4)if(this.status==200){var g=JSON.parse(c.responseText),o;for(o in g)if(g.hasOwnProperty(o)&&o!=="USD")window.currencies[o].rate_usd=g[o];localStorage.currencies=JSON.stringify(window.currencies);Converter.update_currency_display()}else console.error("Request failed.")}}}},Calculator={input:$("#input h1"),output:$("#output h1"),rate:0,add:function(a){this.update_values((this.input.innerText!=="0"?this.strip_commas(this.input.innerText):"")+a)},update_values:function(a){a=this.strip_commas(a);var b=
(a*this.rate).toFixed(2);a||(a=0);if(!/\./.test(a)&&(a.length>5||b.length>6))b=b.slice(0,-3);if(!(a.length>9||b.length>9)){if(/^0?\.*$/.test(a))b="0";this.input.innerHTML=this.add_commas(a);this.output.innerHTML=this.add_commas(b)}},clear:function(){this.update_values("0")},add_commas:function(a){var b=/(\d+)(\d{3,3})/,c=(""+a).split(".");a=c[0];for(c=c[1]!==undefined?"."+c[1]:"";b.test(a);)a=a.replace(b,"$1,$2");return(a||0)+c},strip_commas:function(a){return a.replace(/,/g,"")}},buttons=$("#input-pad p"),
i=0,ii=buttons.length;i<ii;i++)buttons[i].id.length||buttons[i].touch(function(){Calculator.add(this.innerText)});$("#clear").touch(function(){Calculator.clear()});$("#input").touch(function(){addClass($("body"),"edit-rates")});$("#output").touch(function(){addClass($("body"),"edit-rates")});
$("#flip").touch(function(){addClass($("body"),"flip");setTimeout(function(){var a={from:window.from_to.from,to:window.from_to.to};Converter.update_currency_display(a.to,a.from)},130);setTimeout(function(){removeClass($("body"),"flip")},275)});$("#save").touch(function(){removeClass($("body"),"edit-rates")});Converter.draw_currencies();var rates=$("#rate-selection a");i=0;
...
document.addEventListener("load",function(){setTimeout(function(){Converter.update_currencies()},100)},false);
