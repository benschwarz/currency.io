...
/(\d+)(\d{3,3})/,b=(""+a).split(".");a=b[0];for(b=b[1]!==undefined?"."+b[1]:"";c.test(a);)a=a.replace(c,"$1,$2");return a+b},strip_commas:function(a){return a.replace(/,/g,"")}},buttons=$("#input-pad p"),i=0,ii=buttons.length;i<ii;i++)buttons[i].id.length||buttons[i].touch(function(){Calculator.add(this.innerText)});$("#clear").touch(function(){Calculator.clear()});$("#input").touch(function(){$("body").className="edit-rates"});$("#output").touch(function(){$("body").className="edit-rates"});
$("#flip").touch(function(){$("body").className="flip";var a={from:window.from_to.from,to:window.from_to.to};Converter.update_currency_display(a.to,a.from)});$("#save").touch(function(){$("body").className=""});Converter.draw_currencies();var rates=$("#rate-selection a");i=0;for(ii=rates.length;i<ii;i++)rates[i].touch(function(a){a.preventDefault();a=this.id.split("-");args=a[0]=="from"?[a[1],null]:[null,a[1]];Converter.update_currency_display.apply(Converter,args)});
var detectOrientation=function(){if(window.orientation)$("body").className="credits";else $("body").className=""};detectOrientation();window.addEventListener("orientationchange",detectOrientation);if(!navigator.onLine)$("#network-status").className="offline";Converter.update_currency_display();setTimeout(function(){Converter.update_currencies()},100);
